...
define("CRESCENT_WHO_WE_ARE_NID", 9);
define("CRESCENT_CAREERS_NID", 8);
define("PRODUCTS_NID", 16);
define("CRESCENT_SJB_LAUNCH_ENVIROEDGE_NID", 51);

/**
 * Implements hook_preprocess_html().
 */
function crescent_sjb_preprocess_html(&$vars) {
  $html5 = array(
    '#tag' => 'script',
    '#attributes' => array(
      'src' => base_path() . drupal_get_path('theme', 'crescent_sjb') . '/js/lib/html5.js',
    ),
    '#prefix' => '<!--[if (lt IE 9) & (!IEMobile)]>',
    '#suffix' => '</script><![endif]-->',
  );
  drupal_add_html_head($html5, 'crescent_sjb_html5');
  ...
  if ($node = menu_get_object()) {
    switch ($node->type) {
      ...
      case 'resources':
        ...
        break;
      case 'water_services':
        $vars['classes_array'][] = 'page-products';
        break;
      case 'contact':
 ...
 */
function crescent_sjb_preprocess_page(&$vars) {
 ...
 */
function crescent_sjb_preprocess_node(&$vars) {
  //$node = $vars['node'];
  switch ($vars['type']) {
    case 'about_us':
      if (isset($vars['view_mode']) && $vars['view_mode'] == 'full') {
        drupal_goto("node/" . CRESCENT_WHO_WE_ARE_NID);
      }
      break;
    case 'resume':
    case 'application':
      if (isset($vars['view_mode']) && $vars['view_mode'] == 'full') {
        drupal_goto("node/" . CRESCENT_CAREERS_NID);
      }
      break;
    case 'tank_pedestals':
    case 'secondary_containment':
      if (isset($vars['view_mode']) && $vars['view_mode'] == 'full') {
        drupal_goto("node/" . PRODUCTS_NID);
      }
      if ((isset($vars['content']['field_tank_pedestals_images']) && !empty($vars['content']['field_tank_pedestals_images'])) ||
        (isset($vars['content']['field_sec_cont_images']) && !empty($vars['content']['field_sec_cont_images']))
      ) {
    ...
    case 'enviroedge_items':
      ...
      if (isset($vars['node']->nid) && ($vars['node']->nid == CRESCENT_SJB_LAUNCH_ENVIROEDGE_NID)) {
        drupal_add_js('
        function doGo() {
 ...
 */
function crescent_sjb_preprocess_block(&$vars) {
  //kpr($vars);
}
 ...
 */
function crescent_sjb_form_alter(&$form, &$form_state, $form_id) {
  switch ($form_id) {
 ...
 * Build menu.
 */
function crescent_sjb_menu_tree_full_data($menu_name) {
 ...
 * @ingroup themeable
 */
function crescent_sjb_qt_quicktabs_tabset($vars) {
  $variables = array(
    'attributes' => array(
 ...
 */
function crescent_sjb_preprocess_views_view(&$vars) {
  if (isset($vars['view']->name) && $vars['view']->name == "contact") {
    crescent_sjb_prepare_contact_map($vars);
  }
}
 ...
 * @param $vars
 */
function crescent_sjb_prepare_contact_map(&$vars) {
 ...
 */
function crescent_sjb_qt_quicktabs($variables) {
  drupal_add_js(drupal_get_path('theme', 'crescent_sjb') . '/js/quicktabs_dlink.js');
 ...
 */
function crescent_sjb_quicktabs_alter($info) {
 ...
 */
function crescent_sjb_menu_alter(&$items) {
